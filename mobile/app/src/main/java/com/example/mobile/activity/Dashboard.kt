...
class Dashboard : AppCompatActivity(), StoryAdapter.StoryAdapterListener {
    ...
    private fun getAllUserDrawings(userName: String, avatar: String) {
        ...
        call.enqueue(object: retrofit2.Callback<List<IDrawing>> {
            ...
            override fun onResponse(
                ...
                var drawings = arrayListOf<String>()
                var index = 0

                for (drawing in response.body()!!) {
                    if (drawing.isStory == true) {
                        addStory(userName, avatar, drawings)
                        getDrawingData(drawing._id!!, index)
                    }
                    index++
                }
            }
    ...
    private fun addStory(owner: String, avatar: String, drawings: ArrayList<String>) {

        val userAvatarBitmap = bitmapDecoder(avatar)
        val story = IStory(owner, userAvatarBitmap, drawings)
...
//        storyAdapter.notifyItemInserted((rvOutputStories.adapter as StoryAdapter).itemCount)
    ...
    override fun storyAdapterListener(drawingsData: ArrayList<String>) {
        //quand on click sur un avatar, on affiche les drawings
        val intent = Intent(this, Stories::class.java)
        intent.putExtra("drawingsData",drawingsData)
        startActivity(intent)
    }

    private fun getDrawingData(drawingId: String, index: Int) {
        ...
        call.enqueue(object: retrofit2.Callback<IDrawing> {
            ...
            override fun onResponse(call: Call<IDrawing>, response: Response<IDrawing>) {
                ...
                if (currentDrawing != null) {
                    storyAdapter.addDrawingToStory(index, currentDrawing.data!!)
                    storyAdapter.notifyItemInserted((rvOutputStories.adapter as StoryAdapter).itemCount)
                }
            }
